...
export const generateInvoiceForRange = asyncHandler(async (req, res) => {
  ...
  // Create owner if doesn't exist (from token validation)
  if (!owner) {
    await User.create({
      _id: ownerId,
      phone: "unknown",
  ...
  const lineItems = [
    {
      description: `Tiffin service ${billingStart.toDateString()} - ${billingEnd.toDateString()}${
        usedFallback ? " (includes un‑delivered orders)" : ""
      }`,
